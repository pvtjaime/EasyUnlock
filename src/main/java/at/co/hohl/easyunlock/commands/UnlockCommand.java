...
public class UnlockCommand implements CommandExecutor {
  ...
  public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
    if (sender.hasPermission("easyunlock.command.unlock")) {
      if (args.length == 0) {
        ...
        if (data.isRulesAccepted()) {
          ...
          plugin.getServer().broadcast(ChatColor.GREEN + plugin.getConfiguration()
                  .getString("messages.player_unlocked"), "easyunlock.command.unlock");
